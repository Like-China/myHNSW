...
class NSW(object):
    ...
    def __init__(self, distance_type, m=8, ef=128, m0=None, heuristic=True, vectorized=False):
        self.data = []
        if distance_type == "l2":
            # l2 distance
    ...
    def add(self, elem,level, ef=None):
        ...
        '''
        if level is None:
            levelRandom=int(-log2(random()) * self._level_mult) + 1
            if levelRandom>1:
                level=1
            else:
                level=0
    ...
    def search(self, q, k=None, ef=None, coreNodes2Neighbors=None):
        ...
        ep = self._search_graph(q, [(-dist, point)], graphs[1], ef)
        candidates = []
        for  md, idx in ep:
            neighbors = []
            if idx in coreNodes2Neighbors.keys():
                neighbors = coreNodes2Neighbors[idx]
            candidates = list(set(candidates).union(set(neighbors)))
        for e in candidates:
            dist = distance(q, self.data[e])
            heappush(ep, (-dist, e))
        # print("The size of candidates: ", len(candidates))
