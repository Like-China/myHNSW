...
class SCAN:
    ...
    def __init__(self, G, epsilon, mu):
        self._G = G
    ...
    def get_all_core_points(self):
        visit_sequence = list(self._G.nodes())
        coreNodes2Neighbors = {}
        for node_name in visit_sequence:
            node = self._G.nodes[node_name]
            ep_neighbors = self.get_epsilon_neighbor(node_name)
            if len(ep_neighbors) >= self._mu:
                coreNodes2Neighbors[node_name]=ep_neighbors
        return coreNodes2Neighbors
...
def get_communities(pairs, mu, epsilon):
    ...
    return core_nodes, communities, hubs_outliers[0], hubs_outliers[1]
...
if __name__ == '__main__':
    ...
    # 加载自己数据集，每行为一个连边，见数据集示例
    # pairs为一个mX2列表，m为连边树，每个连边由一个两个顶点定义
    pairs = load_data("club.txt")
    
    # 以下是分装好的函数，输入为原kNN连边对，参数mu和参数epsilon
    # 返回: 核心节点聚类中心和组成的社区聚类，桥节点和离群点
    core_nodes, communities, hubs, outliers = get_communities(pairs, mu, epsilon)
    for community in communities:
        print('community: ', sorted(community))
    print("core nodes: ", core_nodes)
    print('hubs: ', hubs)
    print('outliers: ', outliers)
    # 将上述核心节点/桥节点提到HNSW中的上一层，或者核心节点&桥节点提到HNSW中的上一层，试试效果